...
export default function UserDocuments() {
    ...
    useEffect(async () => {
        const paySlipResponse = await get(`${API_URL}user/documents?page=1&limit=20`);
        console.log("=",paySlipResponse?.data?.data)
        setPolicyData(paySlipResponse?.data?.data);
    }, []);
    
    return (
        <Card>
            <Typography variant="h5" sx={{ fontWeight: 600 }}>User Documents</Typography>
            <FilterBox>
                ...
                <Box>
                    <Table>
                        ...
                        <TableBody>
                            ...
                            {/* !policyData... */}
                            {policyData ? policyData?.map((item, key) => (
